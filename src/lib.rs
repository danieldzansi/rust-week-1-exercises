...
pub const CURRENT_BLOCK_HEIGHT: u64 = 800_000;
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    let raw_tx = hex::decode(raw_tx_hex).map_err(|error| format!("Hex decode error: {}", error))?;

    if raw_tx.len() < 4 {
        return Err("Transaction data too short".to_string());
    }

    let version_bytes = [raw_tx[0], raw_tx[1], raw_tx[2], raw_tx[3]];
